...
class PayloadStreamFileSegmentTest(unittest.TestCase):
    def testSmoke(self):
        payloadFile = PayloadStreamSegment.PayloadStreamFile(size=19, index=4, streamOffset=2)
    ...
    def testEndingPayloadStreamOffsetOfFileCalculated(self):
        ...
        fileSegment = PayloadStreamSegment.PayloadStreamFile(size=SAMPLE_FILE_SIZE, index=SAMPLE_FILE_INDEX, streamOffset=SAMPLE_PAYLOAD_SIZE_TO_CURRENT_FILE)
